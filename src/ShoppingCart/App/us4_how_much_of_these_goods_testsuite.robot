...
Calculate Sum For All Checked Goods
    ${sum}    Set Variable    0
    @{list_titles_in_last_page}    Create List    not_exist_title    #预设一个不存在的商品名称，不然系统自带的关键字List Should Contain Value因为空list而报错。
    Set Suite Variable    @{list_titles_in_last_page}
    : FOR    ${i}    IN RANGE    10
    \    ${sum_page}    Calculate Sum For Current Page
...
Calculate Sum For Current Page
    ...
    \    ${sum_one}    Set Variable If    '&{dict_goods_items}[chk]'=='true'    ${sum_one}    0
    \    ${sum}    Evaluate    ${sum} +${sum_one}
    @{list_titles_in_last_page}    Copy List    ${list_titles_in_current_page}    #更新上商品名称列表@{list_titles_in_last_page}，为下一页做准备。
    Set Suite Variable    @{list_titles_in_last_page}
    [Return]    ${sum}
